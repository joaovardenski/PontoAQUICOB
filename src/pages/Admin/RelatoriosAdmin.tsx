...
const RelatoriosAdmin: React.FC = () => {
  ...
  const carregarFuncionarios = async () => {
    try {
      ...
      exibirFeedback("Erro ao carregar funcionários.", "error");
    }
  };

  /** ---------- Funções Auxiliares ---------- */
  const exibirFeedback = (message: string, type: "success" | "error") => {
    setFeedback({ message, type });
  };

  const validarFiltro = (): Funcionario | null => {
    if (!funcionarioSelecionado || !startDate || !endDate) {
      exibirFeedback("Selecione um funcionário e o período para gerar o relatório.", "error");
      return null;
    }
    if (startDate > endDate) {
      exibirFeedback("A data de início não pode ser maior que a data de fim.", "error");
      return null;
    }

    const funcionarioObj = funcionarios.find((f) => f.nome === funcionarioSelecionado) || null;
    if (!funcionarioObj) {
      exibirFeedback("Funcionário não encontrado.", "error");
      return null;
    }
  ...
  const gerarRelatorio = async () => {
    const funcionarioObj = validarFiltro();
    if (!funcionarioObj) return;
    ...
    try {
      ...
      setRelatorioGerado({ funcionario: funcionarioObj, relatorioPorDia, totalPeriodo });
    } catch (error: unknown) {
      exibirFeedback("Erro ao gerar relatório.", "error");
    }
  };
  ...
  const exportarPDF = () => {
    ...
    doc.text(`Meta diária: ${relatorioGerado.funcionario?.carga_horaria ?? 8}h`, 14, 36);
    ...
    autoTable(doc, { head: [["Dia", "Batidas", "Horas"]], body: tableData, startY: 45 });

    const finalY = (doc as any).lastAutoTable?.finalY ?? 60;
    doc.text(`Total no período: ${relatorioGerado.totalPeriodo}`, 14, finalY + 10);
    ...
    <div className="flex flex-col min-h-screen bg-[var(--color-fundo-claro)]">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6 md:p-8 pb-20 md:pb-8">
          <h1 className="hidden md:block text-3xl font-bold mb-6">Relatório de Ponto</h1>
          ...
          <RelatorioFilters
            ...
            onLimpar={limparRelatorio}
          />

          {relatorioGerado ? (
            <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 overflow-x-auto">
              <p className="mb-4 text-sm text-gray-600">
                Relatório gerado para: <strong>{relatorioGerado.funcionario?.nome}</strong> — meta diária: <strong>{relatorioGerado.funcionario?.carga_horaria}h</strong>
              </p>

              <RelatorioTable dias={relatorioGerado.relatorioPorDia} funcionario={relatorioGerado.funcionario!} />
              <RelatorioCards dias={relatorioGerado.relatorioPorDia} funcionario={relatorioGerado.funcionario!} />

              <p className="mt-4 font-semibold">Total no período: {relatorioGerado.totalPeriodo}</p>
            ...
            <p className="text-gray-500 text-center py-6">Nenhum relatório gerado.</p>
          )}
        </main>
